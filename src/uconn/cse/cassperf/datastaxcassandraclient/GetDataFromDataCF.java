...
public class GetDataFromDataCF extends GetRows {
  ...
  /*
   * Start: start pos of query -> id of SQ End: end pos of query -> id + length - 1
   */
...
//    System.out.println(Start + " - " + End);
    String cqlCommand =
        "select * from CassExp.Data where rowName=" + uid + " and columnName >= " + Start
            + " and columnName <= " + End + " limit " + count + ";";
    // System.out.println(cqlCommand);
    return session.execute(cqlCommand);
    // sliceQuery =
    // HFactory.createSliceQuery(keyspace, IntegerSerializer.get(), IntegerSerializer.get(),
