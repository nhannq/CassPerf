...
public class PutData {
	...
	public void generateDataforCassandraDatastax(int uID, int noOfReplicas, int minute, int rate, int startTimeStamp,
			int timeStampInterval, int nbstreams, String consistencyLevel, int maxBatchStmts,
			int testCassBatchStmtPerf) {
		...
		try {
			...
			uconn.cse.cassperf.datastaxcassandraclient.InsertRowsForDataCF iRFCF = new uconn.cse.cassperf.datastaxcassandraclient.InsertRowsForDataCF(
					newRate, timeStampInterval, consistencyLevel, maxBatchStmts);
			Double Value = 0.0;
			...
			System.out.println(
					"Finish putting " + tsID + " " + noOfSamples + " " + rate + " in " + executedTime + " microSec");
			System.out.println("rate:" + rate);
			System.out.println("drop:" + (noOfSamples - tsID));
			System.out.println("ratio:" + tsID / noOfSamples);
	...
	public void generateDataforCassandraAstyanax(int uID, int noOfReplicas, int noOfSamples, int rate) {
		...
		try {
			DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
			uconn.cse.cassperf.astyanaxcassandraclient.InsertRowsForDataCF iRFCF = new uconn.cse.cassperf.astyanaxcassandraclient.InsertRowsForDataCF();
			Double Value = 0.0;
	...
	public void generateDataforCassandraAstyanaxOneClient(int uID, int noOfReplicas, int noOfSamples, int rate) {
